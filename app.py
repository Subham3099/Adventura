...
class User(db.Model):
    ...
    q1       = db.Column(db.Float(precision=3),default=100.0)
    q2       = db.Column(db.Float(precision=3),default=100.0)
    q3       = db.Column(db.Float(precision=3),default=100.0)
    q4       = db.Column(db.Float(precision=3),default=100.0)
    q5       = db.Column(db.Float(precision=3),default=100.0)
    q6       = db.Column(db.Float(precision=3),default=100.0)
    penalty  = db.Column(db.Float(precision=3),default=100.0)
...
def treasure_hunt():
    global current_question, is_started, previous_answer,start_time,cur_user,penalty,q1,q2,q3,q4,q5,q6
            ...
            if answer.lower() == questions[current_question][1].lower():
                if current_question == len(questions) - 1:
                    ...
                    if user:
                        ...
                        total_time = time.time() - start_time + penalty
                    ...
                    if current_question == 0:
                        ...
                        q3 = time.time()-start_time
                    elif current_question == 3:
                        q4 = time.time()-start_time
                    elif current_question == 4:
                        q5 = time.time()-start_time

                    current_question += 1
                    previous_answer = None
                    return render_template('index.html', question=questions[current_question][0])
            else:
                # If the answer is incorrect, or refreshed go to confirm page 
                penalty += 5.0
...
def restart():
    global current_question, is_started, previous_answer,start_time,penalty
    current_question = None
    is_started = False
    previous_answer = None
    start_time = time.time()
